...
// first method, O(log n) time using pre-computed information
// here it is not O(n), I think, because it can be optimized with the kth ancestor method
// where k is the difference of depths;

// 2nd method is based on extended tree traversal array, sometimes called the
// Euler Tour Tree
...
public:
    ...
    // prev not necessary for directed graph
    ...
    int lca(int u, int v)
    {
        ...
        {
            // can be optimised by the kth ancestor method
            // where k is the difference of depths
            if (depth[u] > depth[v])
            {
                u = parent[u];
