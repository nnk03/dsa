...
public:
    ...
    {
        // pair returns {farthest from curr, distance to that node};
        pair<int, int> ans = mp(curr, 0);
        for (auto &neigh : adj[curr])
        {
            if (neigh == prev)
                continue;
            auto p = dfs(neigh, curr);
            if (1 + p.second > ans.second)
            {
                ans.first = p.first;
                ans.second = 1 + p.second;
            }
        }
        return ans;
    }
