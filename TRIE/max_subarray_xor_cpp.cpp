...
public:
    ...
    void insert(int pre_xor)
    {
        ...
        // starting from the MSB, insert all bits of pre_xor into TRIE
        for (int i = INT_SIZE - 1; i > -1; --i)
        {
            bool isSet = pre_xor >> i & 1;
            if (!curr->bits[isSet])
                curr->bits[isSet] = newNode();
            curr = curr->bits[isSet];
        }
        curr->value = pre_xor;
    }
    ...
    {
        // slightly unclear how this works;
        struct TrieNode *curr = root;
        for (int i = INT_SIZE - 1; i > -1; --i)
        {
            bool isSet = pre_xor >> i & 1;
            if (curr->bits[1 - isSet] != NULL)
                curr = curr->bits[1 - isSet];
            else if (curr->bits[isSet] != NULL)
                curr = curr->bits[isSet];
        }
...
void cp()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        pre_xor ^= v[i];
        trie->insert(pre_xor);
        ans = max(ans, trie->query(pre_xor));
    }
    co(ans);
}
